...
angular.module('starter', ['ionic', 'ngCordova', 'starter.controllers', 'starter.services', 'starter.directives', 'starter.filters', 'angular-md5', 'mobiscroll-datetime', 'ngStorage']).constant('BASE_URL', '/Business').constant('BASE_URL', 'http://vmp.witgo.cn/Business').constant('CLIENT_TYPE', 'vehicle_manager').constant('KEY_COMPANY', 'VMP_COMPANY').constant('KEY_TOKEN', 'VMP_TOKEN').constant('KEY_ACCOUNT', 'VMP_ACCOUNT').constant('KEY_USERNAME', 'VMP_USERNAME').constant('KEY_PASSWORD', 'VMP_PASSWORD').run(function($rootScope, $ionicPlatform, $ionicScrollDelegate, $location, $localStorage, $interval, $timeout, KEY_COMPANY, KEY_TOKEN, Account, Message) {
  var checkUpdate, companyCode, oToken, showUpdateConfirm;
  ...
  $rootScope.$on('message.close', function() {
    console.log('message.close');
    return $interval.cancel($rootScope.timer);
  });
  showUpdateConfirm = function(app) {
    var confirmPopup;
    if (app == null) {
      app = {};
    }
    return confirmPopup = $ionicPopup.confirm({
      title: '版本升级',
      template: app.content || ("发现新版本(" + app.version + ")"),
      cancelText: '取消',
      okText: '升级'
    }).then(function(res) {
      var file;
      if (res === true) {
        file = '';
        return $cordovaFileTransfer.download(app.downloadUrl, file, {}, true).then(function() {
          console.log('download success');
          $cordovaFileOpener2.open(file, 'application/vnd.android.package-archive').then(function() {
            return console.log('open success');
          }, function() {
  ...
  checkUpdate = function() {
    return Account.checkUpdate().then(function(app) {
      return $cordovaAppVersion.getAppVersion().then(function(version) {
        if (version !== app.version) {
          return showUpdateConfirm(app);
        }
      });
    });
  };
